...
function gradesAvg(arr) {
    ...
    for(let i=0;i<arr.length;i++){
    const result1 = Object.values(arr[i]);
    const result2 = Object.keys(arr[i]);
      
// console.log(result1);
let total=0;
      let arr1=result1[2].map(function(num){
   total +=num
    //  let num1 =num+num1;
...
returnV.push({firstName:result1[0],lastName:result1[1],gradsList:result1[2],avg:total/8});
...
if(result1[3]<=50)
{
returnV.push({firstName:result1[0],lastNAme:result1[1],gradsList:result1[2],avg:result1[3],result:"Failed"});

}
else{
  
returnV.push({firstName:result1[0],lastNAme:result1[1],gradsList:result1[2],avg:result1[3],result:"Passed"});
